...
Movie.prototype.changeRating = function (number) {
  let rating = (this.rating + number) / 2
  this.rating = rating
  return rating
}
...
savedPosts will be an array of objects. These objects will each have 3 properties: id (a number), title (a string), and rating (a number between 1 and 5). These objects are the posts that the user will have saved to their account.*/
...
/* You will be using the constructor function you just created in problem 5.
 Write a prototype method for the User constructor function named removeSavedPost that will
 take in one number parameter representing the post id. Use this id to find and remove the matching
 object in the savedPosts array.*/

// Code here
User.prototype.removeSavedPost = function (id) {
  const index = this.savedPosts.findIndex(function (post) {
    return post.id === id
  })
